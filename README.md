...
- [ ] 도큐먼트 확장 추가
    - [ ] 모든 도큐먼트가 저장되는 폴더 존재
- [ ] inline link 추가
- [ ] 모든 node가 기본으로 할당되는 페이지 존재
- [ ] 사이드 바
- [ ] 폴더
    - 근데 이건 구조가 확정되어야 우선 진행 할듯
- [ ] 클라우드: 서비스 출시를 고려한 클라우드 서비스랑 나만 쓰는 클라우드 서비스는 또 다른가?
    - [ ] : 
- [ ] 디자인 시스템 도입 (외부 CSS 사이드로드): 테마(다크모드) 및 커스텀 폰트/스타일 지원.
- [ ] 코드 리팩토링: Store 로직 분리 및 컴포넌트 최적화.
- [ ] 외부 연동 (Integrations):
    - [ ] 이미지 업로드/붙여넣기.
    - [ ] 마크다운 내보내기.
- [ ] 멀티 플랫폼 확장:
    ...
    - [ ] PWA: 모바일 설치 지원.
- [ ] 협업 기능 (Real-time): Yjs 등을 활용한 동시 편집 (로컬 우선 동기화).